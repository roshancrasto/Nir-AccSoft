...
IF OBJECT_ID('NU_sp_Report_AccountLedger', 'P') IS NOT NULL
    DROP PROCEDURE NU_sp_Report_AccountLedger;
GO
...
BEGIN
    ...
    BEGIN
        ...
        SELECT ISNULL(r.ReceivedDate, r.ReceiptDate), 'Receipt No ' + ISNULL(r.ReceiptNumber, '') + ISNULL(' - ' + d.DonorName, ''), r.ReceiptNumber, r.CashAmount
        FROM NU_Receipts r 
        ...
        WHERE DeletedFlag = 0 AND ConsiderForAudit = 1 AND CollectionDate BETWEEN @FromDate AND @ToDate;
        ...
        SELECT p.PaymentDate, ISNULL(p.VoucherNumber, '') + ISNULL(' - ' + v.VendorName, '') + ISNULL(' - ' + p.Description, ''), p.VoucherNumber, p.DebitAmount
        FROM NU_Payments p
        LEFT JOIN NU_Vendors v ON p.VendorId = v.VendorId
        WHERE p.DeletedFlag = 0 AND p.PaymentMode = 'Cash' AND p.PaymentDate BETWEEN @FromDate AND @ToDate;
    END
    ELSE IF @AccountType = 'Bank'
    BEGIN
        -- Receipts
        INSERT INTO #Ledger (Date, Particulars, ReferenceNo, Receipt)
        SELECT ISNULL(r.ReceivedDate, r.ReceiptDate), 'Receipt No ' + ISNULL(r.ReceiptNumber, '') + ISNULL(' - ' + d.DonorName, ''), r.ReceiptNumber, r.BankAmount
        FROM NU_Receipts r 
        LEFT JOIN NU_Donors d ON r.DonorId = d.DonorId
        ...
        FROM NU_Payments p
        LEFT JOIN NU_Vendors v ON p.VendorId = v.VendorId
        WHERE p.DeletedFlag = 0 AND p.PaymentMode IN ('Cheque', 'Online', 'Bank', 'DD') AND p.PaymentDate BETWEEN @FromDate AND @ToDate;
    END
    ...
    INSERT INTO #Ledger (Date, Particulars, ReferenceNo, Payment)
    SELECT TransferDate, 'Transfer To ' + ISNULL(ToAccountType, 'Account'), ISNULL(TransferNo, ''), Amount
    FROM NU_BankTransfers 
